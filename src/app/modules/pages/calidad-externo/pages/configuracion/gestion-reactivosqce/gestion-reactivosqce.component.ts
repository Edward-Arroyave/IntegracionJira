...
export class GestionReactivosqceComponent implements OnInit {
  ...
  displayedColumns: string[] = ['Reactivo', 'Estado', 'Editar', 'Eliminar'];
  ...
  cargarGestionReactivos() {
    this.reactivosQceService.getAllAsync().then(respuesta => {
      ...
      this.dataTableBody = filtrarDataTable.map(x => {
        return { Reactivo: x.desreagents, Estado: x.active, item: x, item3: x, item4: x };
      });
  ...
  openModalGestionReactivos(templateGestionReactivos: TemplateRef<any>, datos: any) {
    ...
    dialogRef.componentInstance.primaryEvent?.pipe(takeUntil(destroy$)).subscribe(x => {
      ...
      this.crearEditarGestionReactivos();
      dialogRef.close();
    });
  }
  ...
  actualizarEstadoGestionReactivos(datosReactivos) {
    ...
    const datos = { idreagents: data.idreagents, desreagents: data.desreagents, active: estado }

    this.reactivosQceService.update(datos, data.idreagents).subscribe(respuesta => {
      this.cargarGestionReactivos();
      this.accion = 'Editar';
  ...
  eliminarGestionReactivos(id: any) {
    this.reactivosQceService.delete('Reagents', id).subscribe({
      next: (respuesta) => {
        this.cargarGestionReactivos();
        this.accion = '';
